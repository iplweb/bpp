...
2.  Aby utworzyć raport slotów - uczelnia przez API za pomocą polecenia `curl(1)`,
...
4.  Dane raportu zwracane są asynchronicznie. Oznacza to, że dopóki raport nie otrzyma wartości
...
5.  Następnie możliwe będzie pobranie pojedynczych wierszy tego raportu. Przez stronę WWW
...
6.  dane w formacie JSON są stronnicowane, po kilka wpisów na stronę, aby nie przeciążać serwera. Warto zwrócić
...
2.  **Pobranie konkretnego wydawnictwa ciągłego:**
...
3.  **Filtrowanie wydawnictw po roku:**
...
4.  **Pobranie wydawnictw zmienionych w określonym okresie:**
...
2.  **Pobieranie z filtrowaniem po roku:**
    - Method: GET
    - URL: `https://adres.serwera/api/v1/wydawnictwo_ciagle/`
    - Params:
      - `rok__gte`: `2020`
      - `rok__lte`: `2023`
3.  **Pobieranie z paginacją:**
    - Method: GET
    - URL: `https://adres.serwera/api/v1/wydawnictwo_ciagle/`
    - Params:
      - `page`: `2`
      - `page_size`: `50`
4.  **Pobieranie konkretnego rekordu:**
    - Method: GET
