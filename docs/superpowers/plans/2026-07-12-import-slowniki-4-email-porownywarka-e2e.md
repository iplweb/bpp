...
/ stanowisko dydaktyczne) w tabeli podglądu — różnica PODŚWIETLONA, bez
nadpisywania istniejącego; (3) rozszerzenie etykiety `ZAKRES_STRUKTURA` o
stopnie/stanowiska; (4) test E2E spinający komórkę złożoną → utworzenie jednostek
(Krok 1) → import osób ze stopniem/stanowiskiem/e-mailem (Krok 2); (5) newsfragment
...
`select_related`. Etykieta zakresu to zmiana `choices` (migracja `AlterField`).
...
(§11 e-mail, §12 porównywarka, §14 testy/E2E, §16 poza zakresem). Uwaga: w chwili
pisania planu spec i Plany 1/3 tej serii nie istnieją jeszcze pod docelowymi
nazwami — interfejsy oparto na Planie 2
(`2026-07-12-import-slowniki-2-common-parsery-mapowanie.md`) oraz na kontrakcie
nazw poniżej.

**Zależność:** wymaga Planu 1 (pola `Autor.email`, `Autor.stopien_sluzbowy`,
`Autor_Jednostka.stanowisko`, modele `StopienSluzbowy`/`StanowiskoDydaktyczne`),
Planu 2 (klasyfikatory + `parsuj_komorke` + cele mapowania `email`/
`stopień_służbowy`/`stanowisko_dydaktyczne`/`komórka_złożona`/`nazwisko_imię`) oraz
Planu 3 (wpięcie parsera komórki, klasyfikatorów i e-maila/stopnia/stanowiska w
`analyze`/`integrate`, dodanie `email` do `AutorForm`). Task 5 (E2E) jest testem
akceptacyjnym CAŁEGO feature'u — przechodzi dopiero po ukończeniu Planów 1–3.
...
- **NIE modyfikować WYDANYCH migracji.** Świeże migracje na `dev` (niewydane) można
  edytować, ale w tym planie GENERUJEMY nową (`AlterField` etykiety zakresu) i jej
  nie ruszamy po wygenerowaniu.
- **E-mail no-overwrite dla istniejących:** import USTAWIA `Autor.email` tylko dla
  ...
  Planu 3 — tu tylko testujemy i pokazujemy różnicę). Porównywarka jest read-only.
...
- Modify: `src/import_pracownikow/models.py` — `ImportPracownikowRow.porownaj_z_baza`,
  `select_related` w `get_details_set`, etykieta `ZAKRES_STRUKTURA` + help_text.
- Create: `src/import_pracownikow/migrations/0023_alter_importpracownikow_zakres_integracji.py`
  (generowana przez `makemigrations`).
...
- Modify: `src/import_pracownikow/templates/import_pracownikow/przeglad.html` — sufiks
  przycisku „Zapisz jednostki + tytuły".
- Testy: `src/import_pracownikow/tests/test_parsers/test_oczysc_email.py`,
  ...
  `src/import_pracownikow/tests/test_pipeline/test_e2e_slowniki.py`; rozszerzenie
...
- Produces: `oczysc_email(dane: dict) -> str | None` — mutuje `dane["email"]` na
  ...
  ostrzeżenia (gdy adres był niepusty i niepoprawny) albo `None`. Czysta funkcja
  (bez ORM), nie rzuca.
...
def oczysc_email(dane: dict):
    ...
    (openpyxl) potrafi dać komórkę nietekstową. Nie rzuca (per-wiersz recovery)."""
...
- Consumes: `row.dane_znormalizowane` (klucze `email`/`stopień_służbowy`/
  `stanowisko_dydaktyczne`), `row.autor.email`, `row.autor.stopien_sluzbowy`
  (FK, Plan 1), `row.autor_jednostka.stanowisko` (FK, Plan 1).
- Produces: `ImportPracownikowRow.porownaj_z_baza() -> dict` z kluczami
  `email`/`stopien`/`stanowisko`, każdy = `{"plik": str, "baza": str,
  "rozne": bool}`; dwa nowe `select_related` w `get_details_set`; trzy kolumny w
  tabeli podglądu (nagłówki + komórki) z podświetleniem różnicy.
...
def test_porownaj_z_baza_wykrywa_roznice_emaila():
    ...
    row = ImportPracownikowRow.objects.create(
        ...
        dane_znormalizowane={
            ...
            "stopień_służbowy": "kpt.",         # zgodny
            "stanowisko_dydaktyczne": "prof.",  # różni się
        },
...
```python
    @staticmethod
    def _porownaj(plik, baza):
        """Trójka porównania jednego pola: ``{plik, baza, rozne}``. ``rozne`` =
        obie strony NIEPUSTE i różne (case-insensitive) — pole puste w pliku LUB
        w bazie NIE jest różnicą (import niczego nie nadpisuje pustym, a puste w
        bazie to po prostu uzupełnienie)."""
        p = str(plik or "").strip()
        b = str(baza or "").strip()
    ...
    def porownaj_z_baza(self):
        ...
        nadpisuje (no-overwrite jest w integracji). Strona pliku:
        ``dane_znormalizowane``; strona bazy: FK autora / powiązania. Stanowisko
        dydaktyczne czytamy z ``autor_jednostka`` (powiązanie aktualizowane przez
        ten wiersz); dla wiersza bez autora/AJ strona bazy jest pusta."""
        dane = self.dane_znormalizowane or {}
        autor = self.autor
        aj = self.autor_jednostka
        email_baza = autor.email if autor else ""
        stopien_baza = (
            str(autor.stopien_sluzbowy)
            if autor and autor.stopien_sluzbowy_id
            else ""
        )
        stanowisko_baza = (
            str(aj.stanowisko) if aj and aj.stanowisko_id else ""
        )
        return {
            "email": self._porownaj(dane.get("email"), email_baza),
            "stopien": self._porownaj(
                dane.get("stopień_służbowy"), stopien_baza
            ),
            "stanowisko": self._porownaj(
                dane.get("stanowisko_dydaktyczne"), stanowisko_baza
            ),
        }
```
...
{# import-porownanie-rozne); import NIE nadpisuje istniejącego (no-overwrite w #}
{# integracji) — to tylko sygnał dla operatora. #}
{% with porownanie=row.porownaj_z_baza %}
    <td>{% include "import_pracownikow/partials/_porownanie_kom.html" with pole=porownanie.email %}</td>
...
{# sama wartość z pliku (albo „—"). #}
{% if pole.rozne %}
    <span class="label warning import-porownanie-rozne">{{ pole.plik }}</span>
    <br>
    <span class="secondary">baza: {{ pole.baza }}</span>
{% elif pole.plik %}
    {{ pole.plik }}
{% else %}
...
## Task 4: Etykieta `ZAKRES_STRUKTURA` + przycisk (stopnie/stanowiska)
...
- Modify: `src/import_pracownikow/tests/test_przeglad.py`

**Interfaces:**
- Consumes: `ImportPracownikow.ZAKRES_CHOICES`, `ZAKRES_STRUKTURA`.
- Produces: rozszerzona etykieta zakresu (o „stopnie/stanowiska") + spójny sufiks
  przycisku na hubie; migracja `AlterField`.

- [ ] **Step 1: Write the failing test**

Dodaj do `src/import_pracownikow/tests/test_przeglad.py` (na końcu pliku):

```python
def test_etykieta_zakresu_struktura_wymienia_stopnie_stanowiska():
    from import_pracownikow.models import ImportPracownikow

    etykieta = dict(ImportPracownikow.ZAKRES_CHOICES)[
        ImportPracownikow.ZAKRES_STRUKTURA
    ]
    assert "stopnie" in etykieta.lower()
    assert "stanowiska" in etykieta.lower()
```

- [ ] **Step 2: Run test to verify it fails**

Run: `uv run pytest src/import_pracownikow/tests/test_przeglad.py::test_etykieta_zakresu_struktura_wymienia_stopnie_stanowiska -v`
Expected: FAIL — etykieta to nadal „jednostki + tytuły (bez osób)".

- [ ] **Step 3: Zmień etykietę + help_text + komentarz w modelu**

W `src/import_pracownikow/models.py`:

Komentarz (linia ~86) — z:
`    # - STRUKTURA: jednostki + tytuły (bez osób).`
na:
`    # - STRUKTURA: jednostki + tytuły + stopnie/stanowiska (bez osób).`

Wpis `ZAKRES_CHOICES` — z:
`        (ZAKRES_STRUKTURA, "jednostki + tytuły (bez osób)"),`
na:
`        (ZAKRES_STRUKTURA, "jednostki + tytuły + stopnie/stanowiska (bez osób)"),`

`help_text` pola `zakres_integracji` — z:
`        "(struktura + osoby), same jednostki, albo jednostki + tytuły "`
`        "(bez osób). Ustawiane przez przycisk zatwierdzenia na hubie.",`
na:
`        "(struktura + osoby), same jednostki, albo jednostki + tytuły + "`
`        "stopnie/stanowiska (bez osób). Ustawiane przyciskiem na hubie.",`

- [ ] **Step 4: Wygeneruj migrację**

Run: `uv run python src/manage.py makemigrations import_pracownikow`
Expected: powstaje `0023_alter_importpracownikow_zakres_integracji.py` (AlterField
`zakres_integracji`). NIE edytuj jej ręcznie.

Sanity: `uv run python src/manage.py makemigrations --check --dry-run import_pracownikow`
→ „No changes detected" (migracja domyka drift `choices`).

- [ ] **Step 5: Rozszerz przycisk na hubie (spójność z etykietą)**

W `src/import_pracownikow/templates/import_pracownikow/przeglad.html`, przycisk
„Zapisz jednostki + tytuły" — dopisz SUFIKS (zachowując frazę bazową jako
podłańcuch, żeby istniejące asercje `test_przeglad.py` zostały zielone):

```django
                        <span class="fi-crown"></span>
                        Zapisz jednostki + tytuły + stopnie/stanowiska
```

- [ ] **Step 6: Run tests to verify they pass**

Run: `uv run pytest src/import_pracownikow/tests/test_przeglad.py -v`
Expected: PASS (nowy test etykiety + istniejące asercje przycisku — „Zapisz
jednostki + tytuły" nadal jest podłańcuchem obecnego/nieobecnego przycisku).

- [ ] **Step 7: Commit**

```bash
git add src/import_pracownikow/models.py \
  src/import_pracownikow/migrations/0023_alter_importpracownikow_zakres_integracji.py \
  src/import_pracownikow/templates/import_pracownikow/przeglad.html \
  src/import_pracownikow/tests/test_przeglad.py
git commit -m "feat(import_pracownikow): zakres struktura obejmuje stopnie/stanowiska (etykieta+przycisk)"
```

---

## Task 5: Test E2E — komórka złożona → jednostki (Krok 1) → osoby (Krok 2)
...
def test_e2e_komorka_zlozona_stopnie_stanowiska_email(uczelnia):
    ...
    wiersze = [
        ...
        _wiersz(1, KOMORKA_A, "Anszczak Marcin", "marcin@example.com",
                "kpt.", "adiunkt"),
        # ISTNIEJĄCY autor — e-mail NIE nadpisany (no-overwrite §11).
        _wiersz(2, KOMORKA_B, "Kowalski Jan", "nowy@example.com",
                "kpt.", "adiunkt"),
    ]
...
## Task 6: Newsfragment (towncrier) — podsumowanie feature'u
...
Jeśli fragment już istnieje (dodany w Planie 1/2/3) — pomiń Task 6, ewentualnie
...
- [ ] **Step 5: (przy scalaniu serii) odśwież baseline RAZ**

Migracja `0023` zmienia `choices` — po scaleniu Planów 1–4 do `dev` odśwież
baseline JEDEN raz (nie w równoległych branchach): `make baseline-update`, commit
...
ostrzeżenie w wierszu, bez crasha analizy); tabela podglądu ma porównywarkę
„plik vs baza" dla e-maila/stopnia/stanowiska z podświetleniem różnic (read-only,
no-overwrite); etykieta zakresu „struktura" i przycisk huba wymieniają
stopnie/stanowiska; test E2E spina komórkę złożoną → utworzenie jednostek → import
osób ze stopniem/stanowiskiem/e-mailem; newsfragment podsumowuje cały feature.
Seria słowników stopień/stanowisko (Plany 1–4) domknięta.
