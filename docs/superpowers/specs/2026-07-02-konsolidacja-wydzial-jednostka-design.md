...
potomków. W multiseek `WydzialQueryObject` / `PierwszyWydzialQueryObject`
zwijają się do zapytań po `Jednostka` + `get_family()`/`get_descendants()`
(drzewo już to wspiera).
...
8. `RodzajJednostki` edytowalny per-tenant; koło naukowe = rodzaj z flagą
   `wyklucz_z_rankingu_autorow` (te same wykluczenia z rankingu co dotąd).
9. Cała suita testów zielona; baseline odświeżony.
10. Migracja idempotentna i przetestowana na ≥1 realnym dumpie multi-tenant.
