...
- **Wersje (bumpver, wzorzec `vYYYY0M.BUILD[-TAGNUM]`):**
  - pierwszy cut-RC: `bumpver update --tag rc --tag-num` → z `…1391` (final) na
    `…1392-rc1` (inkrementuje BUILD i dokleja `-rc1`; identyczny mechanizm co
    `make bump-dev`, tylko tag `rc` zamiast `dev`).
  - kolejny cut-RC (gałąź `release/*` już istnieje): `bumpver update --tag-num`
    → `-rc2`, `-rc3`.
  - promote: `bumpver update --tag final` → zdejmuje `-rc` → `…1392`. **Nie
    re-datuje** (zachowuje `YYYY0M` z current_version), więc RC z czerwca
    promowany w lipcu pozostaje `202606.1392`.
...
1. Ustal gałąź wydania:
   - brak otwartej `release/*` → policz BASE (preview `bumpver update --dry`),
     `git switch -C release/v<BASE> origin/dev`, `bumpver update --tag rc
     --tag-num` (rc1).
   - istnieje otwarta `release/*` → checkout jej, `git merge --no-ff origin/dev`
     (podbiera fixy z dev — patrz §8 caveat o scope-creep), `bumpver update
     --tag-num` (rc2…).
...
3. `bumpver update --tag final` → `202606.1392`. Commit.
