-- Pożegnanie z kolacją libc public."pl_PL" (REVERSE / odtworzenie).
--
-- UWAGA: ten reverse odtwarza kolację libc pl_PL.UTF-8, więc DZIAŁA TYLKO na
-- obrazie, który ma wygenerowane locale libc pl_PL.UTF-8 (stary
-- iplweb/bpp_dbserver albo własny obraz z `localedef ... pl_PL.UTF-8`).
-- Na stockowym postgresie CREATE COLLATION padnie ("could not create locale").
--
-- Lustro forwardu: tworzymy kolację, kasujemy łańcuch widoków (od góry) i
-- odtwarzamy go Z klauzulą COLLATE "pl_PL" (od dołu).

CREATE COLLATION IF NOT EXISTS public."pl_PL" (locale = 'pl_PL.UTF-8');
