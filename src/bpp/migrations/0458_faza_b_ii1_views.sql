...
-- Faza B / issue #438 — II-1 (a): przebudowa widoków sum rankingu autorów.
--
-- Zdejmujemy JOIN do bpp_wydzial. Zachowujemy kolumnę
-- ``bpp_jednostka.wydzial_id AS wydzial_id`` — w tym momencie (PRZED
-- retargetem kolumny w migracji 0459) trzyma ona jeszcze stary Wydzial.id,
-- więc zdjęcie JOIN-u jest bezpieczne. Po 0459 ta sama kolumna zacznie
-- trzymać Jednostka-korzeń (self-FK) i widok automatycznie to odzwierciedli.
--
-- Reguła członkostwa = ``wchodzi_do_rankingu_autorow = TRUE`` (pole po
-- RenameField z 0454; dawniej ``wchodzi_do_raportow``). Świadome
-- rozluźnienie (Kryterium #3): jednostki bez wydziału (wydzial_id IS NULL)
-- NIE są już wykluczane przez INNER JOIN — wliczają się, o ile nie
-- odznaczono ``wchodzi_do_rankingu_autorow``.
...
SELECT ARRAY [bpp_autor.id, bpp_jednostka.id] AS id,
       ...
       bpp_jednostka.wydzial_id               AS wydzial_id,

       rok                                    AS rok,
       punktacja_wewnetrzna                   AS punktacja_wewnetrzna,
       punktacja_snip                         AS punktacja_snip,
...
WHERE bpp_autor.id = bpp_patent_autor.autor_id
  ...
  AND bpp_jednostka.wchodzi_do_rankingu_autorow = TRUE;
...
FROM bpp_autor
         JOIN bpp_praca_habilitacyjna
              ON bpp_autor.id = bpp_praca_habilitacyjna.autor_id
         JOIN bpp_jednostka
              ON bpp_jednostka.id = bpp_praca_habilitacyjna.jednostka_id
         JOIN bpp_typ_kbn
              ON bpp_typ_kbn.id = bpp_praca_habilitacyjna.typ_kbn_id

WHERE bpp_jednostka.wchodzi_do_rankingu_autorow = TRUE
  AND bpp_typ_kbn.skrot != 'PW';
