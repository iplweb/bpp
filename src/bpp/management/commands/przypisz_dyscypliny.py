...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for ad in Autor_Dyscyplina.objects.all().filter(subdyscyplina_naukowa=None).exclude(
                dyscyplina_naukowa=None).select_related():
            for klass in Wydawnictwo_Zwarte_Autor, Wydawnictwo_Ciagle_Autor:
                for instance in klass.objects.filter(autor=ad.autor, rekord__rok=ad.rok, dyscyplina_naukowa=None).select_related("rekord",):
