...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for praca in Wydawnictwo_Ciagle.objects.filter(
            rok__gte=2016, charakter_formalny__skrot="AC", punkty_kbn__gt=0
        ).only("pk", "punkty_kbn", "tytul_oryginalny", "zrodlo_id", "rok"):
            try:
                pz = Punktacja_Zrodla.objects.get(rok=praca.rok, zrodlo=praca.zrodlo_id)
