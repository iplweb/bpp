...
class TabelkaZImpactem(TabelaRaportuKomisjiCentralnejMixin, SumyImpactKbnMixin,
                       Table):
    class Meta:
        attrs = {"class": "z_ramka"}
    ...
    def __init__(self, *args, **kwargs):
        ...
        TabelaRaportuKomisjiCentralnejMixin.__init__(self)
...
class TabelkaBezImpactu(TabelkaZImpactem):
    class Meta:
        template = "raporty/raport_komisji_centralnej/tabela_ib.html"
...
class Tabelka9Mixin:
    def _value(self, record, name):
        kc = getattr(record, 'kc_' + name)
        if kc:
            return kc
        return getattr(record, name)

    def render_impact_factor(self, record):
        value = self._value(record, 'impact_factor')
        self.sum_impact += value
        return value

    def render_punkty_kbn(self, record):
        value = self._value(record, 'punkty_kbn')
        self.sum_punkty_kbn += value
        return value


class TabelkaNr9A(Tabelka9Mixin, TabelkaZImpactem):
    class Meta:
        template = "raporty/raport_komisji_centralnej/tabela_ia.html"
        attrs = {"class": "z_ramka"}
        pass
...
class TabelkaNr9B(Tabelka9Mixin, TabelkaBezImpactu):
    class Meta:
        template = "raporty/raport_komisji_centralnej/tabela_ib.html"
        attrs = {"class": "z_ramka"}
        pass
...
def get_queries(autor, przed_habilitacja=True, rok_habilitacji=None):
    ...
    ret = {
        ...
        '9a': base_query.filter(
            ...
            **kw9).order_by(*order_if_kc),

        '9b': base_query.filter(
            Q(Q(impact_factor=0) & Q(
                Q(kc_impact_factor=0) | Q(kc_impact_factor=None))),
            **kw9).order_by(*order_kbn),
...
class RaportKomisjiCentralnej:
    def __init__(self, autor, przed_habilitacja=True, rok_habilitacji=None):
        ...
        for key in ['1a', '1b', '2a', '2b', '3a', '3b']:
            result = queries[key].aggregate(Sum('impact_factor'),
                                            Sum('punkty_kbn'))
    ...
    def policz_sumy(self):
        ...
        def sum_dict(*prefixes):
            ...
            return dict(count=count, punkty_kbn=punkty_kbn,
    ...
    def punktacja_sumaryczna(self, partial=False):
        ...
        template_name = "raporty/raport_komisji_centralnej/" \
                        "punktacja_pojedyncza.html"
        if partial:
