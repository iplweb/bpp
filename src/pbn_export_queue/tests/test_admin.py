...
def test_pbn_export_queue_admin_changelist_loads(admin_client):
    ...
    url = reverse("admin:pbn_export_queue_pbn_export_queue_changelist")
...
def test_pbn_export_queue_admin_resend_single_item(wydawnictwo_ciagle, admin_user):
    ...
    with patch("pbn_export_queue.tasks.task_sprobuj_wyslac_do_pbn.delay") as mock_task:
...
def test_pbn_export_queue_admin_response_change_resend(
    ...
    with middleware(request):
        with patch(
            ...
            assert (
                f"/admin/pbn_export_queue/pbn_export_queue/{queue_item.pk}/change/"
                in response.url
            )
