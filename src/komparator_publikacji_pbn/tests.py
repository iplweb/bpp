...
@pytest.mark.django_db
def test_publication_comparison_view_requires_staff(admin_user, normal_user):
...
def test_publication_comparison_view_with_matching_data(admin_user):
    ...
    pbn_pub = baker.make(
        ...
        doi="10.1234/test",
        status="ACTIVE",
        versions=[{"current": True, "object": {"title": "Test Publication", "year": 2023}}]
    )
...
def test_publication_comparison_view_with_differences(admin_user):
    ...
    pbn_pub = baker.make(
        ...
        doi="10.1234/pbn",
        status="ACTIVE",
        versions=[{"current": True, "object": {"title": "PBN Title", "year": 2023, "doi": "10.1234/pbn"}}]
    )
...
def test_publication_comparison_isbn_for_books(admin_user):
    ...
    pbn_book = baker.make(
        ...
        year=2023,
        status="ACTIVE",
        versions=[{"current": True, "object": {"title": "Book Title", "isbn": "978-83-1234567-8-9"}}]
    )
...
def test_publication_comparison_author_count(admin_user):
    ...
    baker.make("bpp.Wydawnictwo_Ciagle_Autor", rekord=bpp_pub, autor=autor1)
    baker.make("bpp.Wydawnictwo_Ciagle_Autor", rekord=bpp_pub, autor=autor2)
