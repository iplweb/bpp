...
if [ "$ENABLE_AUTORELOAD_ON_CODE_CHANGE" = "1" ] || [ "$ENABLE_AUTORELOAD_ON_CODE_CHANGE" = "true" ]; then
    ...
    exec uv run watchmedo auto-restart --directory=/app/src --pattern=*.py --recursive -- uv run celery -A django_bpp.celery_tasks beat --pidfile=/celerybeat.pid -s /celerybeat-schedule
else
