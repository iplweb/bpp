...
if [ "$USE_DEV_TEMPLATE" = "true" ] || [ "$USE_DEV_TEMPLATE" = "1" ]; then
    echo "Using development nginx configuration (auto-generates certificates if needed)"
    # Use development template that handles missing certificates gracefully
    cp /etc/nginx/templates/default.conf.dev.template /etc/nginx/templates/default.conf.template
    ...
    echo "Using production nginx configuration (requires existing certificates)"
    # Use production template that requires certificates
    # The original default.conf.template is already in place
